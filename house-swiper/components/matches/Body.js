...
export default function Body(){
    ...
    useEffect( () => { 
        async function fetchData() {
            try {
            const response = await axios.get(`https://${process.env.SERVER_URI}/profiles/matches`,{ withCredentials: true }, { headers: {'Content-Type': "application/json"}})     
                ...
                {data.map((person,index) => (
                 <Match
                    ...
                    unmatchHandler = { async ()=> {
                        const res = await await axios.patch(`https://${process.env.SERVER_URI}/matching/unmatch/`,{username:person.username},{withCredentials:true},{ headers: {'Content-Type': "application/json"}})
                        if(res.data == "success"){
